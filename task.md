...
## NOCH OFFEN
-1. **P-27/P-28-Fix "individuelle Varianten-Preise" (2026-09-08/09):** PR #77 (Code-Fix), PR #78 (Reparatur-Endpunkt) und PR #79 (Chunking nach Produktzahl) **GEMERGT**. Live-Test 09.09. ~16:41 Uhr zeigte: `repairVariantPricesForProduct()` arbeitet weiterhin korrekt, aber "5 Produkte pro Batch" reichte nicht — ein Produkt (71) mit allein 11 Varianten sprengte das Zeitbudget einer Charge (15 SKU-Updates, 39s Verarbeitungszeit laut Render-Logs, kein Absturz, trotzdem 502 vom Render-Verbindungs-Timeout). **PR #81 (Draft, wartet auf manuelle Merge-Freigabe):** `computeRepairBatchRange()` von produkt- auf varianten-/SKU-gewichtete Chargen umgestellt (`maxVariantsPerBatch`, Default 6) — ein Produkt über dem Limit bildet notfalls seine eigene größere Charge statt mit anderen zusammengelegt zu werden. Frontend unverändert (geprüft: `offset += results.length` bleibt korrekt). Committeter Test bildet das Live-Fund-Szenario (3/11/2/1/4/2 Varianten) exakt nach. Typecheck + `bun test` (14/14) grün. **Nach Merge von PR #81:** Nutzer führt die Reparatur erneut aus (sollte jetzt auch bei Produkt 71 ohne Timeout durchlaufen), danach stele-138 final verifizieren. Verwandtes, separat angefragtes PR 2/2 (analoger Fix in `/products/check-all-prices`, das aktuell GAR KEINE Varianten-Erkennung hat) weiterhin nicht begonnen.
