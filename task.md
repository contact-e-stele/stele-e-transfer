...
## NOCH OFFEN
-1. **P-27/P-28-Fix "individuelle Varianten-Preise" (2026-09-08/09):** PR #77 (Code-Fix, `updateEbayVariantPricesIndividually()`) **GEMERGT**. Live-Nachtest 09.09. zeigte: Fix behebt den Bug für künftige Preisänderungen, repariert aber bereits falsch bepreiste Listings NICHT rückwirkend (stele-138 weiterhin "EUR 20,95-EUR 20,95") — Ursache: `recalculate-preview`s 0,50€-Diff-Schwelle zeigt betroffene Listings bei unverändertem Einkaufspreis nie an. **PR #78 (Draft, wartet auf manuelle Merge-Freigabe):** neuer Endpunkt `POST /ebay/listings/repair-variant-prices` + Button "Varianten-Preise reparieren (einmalig)" im Listings-Tab — läuft OHNE Schwellenwert-Prüfung über alle live gelisteten Varianten-Produkte. `repairVariantPricesForProduct()` (price-monitor.ts) als testbare Kernlogik extrahiert. Typecheck + `bun test` (9/9) grün. **Nach Merge von PR #78:** Nutzer führt die Reparatur einmal für alle 31 aktuell live gelisteten Produkte aus, danach stele-138 live verifizieren — danach nicht mehr nötig, künftige Preisänderungen laufen über den regulären "Preise neu berechnen"-Weg. Verwandtes, separat angefragtes PR 2/2 (analoger Fix in `/products/check-all-prices`, das aktuell GAR KEINE Varianten-Erkennung hat) weiterhin nicht begonnen.
