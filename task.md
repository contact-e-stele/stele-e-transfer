...
## NOCH OFFEN
0. **P-112 (2026-09-07, AKTIV, KURSÄNDERUNG):** Verkäufer-Erkennung (P-109/P-110/P-111) zeigt live weiterhin "(kein Name erkannt)" bei Produkt 1005010280178344 trotz Fix im DS-API-Pfad (#71 deployed). Nutzer hat gestoppt: keine weiteren blinden Fix-Versuche mehr — stattdessen erst die ECHTEN Rohdaten sehen. Umgesetzt: neue Diagnose-Logs in `scrapeWithDsApi()` (aliexpress.ts) — (a) `collectShopRelatedFields()` findet automatisch JEDES Feld (String ODER Zahl) irgendwo in der DS-API-Antwort, dessen KEY "shop"/"seller"/"store" enthält (deckt auch numerische ID-Felder wie `sellerId` ab, nicht nur Namensfelder), (b) für GENAU dieses eine Testprodukt zusätzlich kompletter, ungekürzter Rohdaten-Dump. Kein funktionaler Fix in diesem Schritt — nur Diagnose, wie explizit gefordert. **Wartet auf:** Nutzer muss deployen, Produkt 1005010280178344 erneut scrapen, und die beiden neuen Log-Zeilen (`[DS API] P-112-Diagnose: N shop/seller/store-bezogene Felder gefunden: [...]` sowie den vollständigen Dump zwischen den START/ENDE-Markern) aus Render kopieren. Erst danach: falls eine numerische Shop-ID existiert → Umstellung der Lieferanten-Verifizierung von Namens-Fuzzy-Match auf exakten ID-Vergleich (ID beim manuellen Shop-Anlegen aus der URL extrahieren); falls keine Shop-Kennung in der DS-API-Antwort existiert → kompletter Wechsel auf einen anderen Datenweg (z.B. dedizierter Shop-Info-API-Call) statt weiterer Versuche im Produkt-Response.
