...
## NOCH OFFEN
-1. **P-27/P-28-Fix "individuelle Varianten-Preise" (2026-09-08, PR #77 Draft, wartet auf manuelle Merge-Freigabe):** Live-Fund stele-138 (3 Varianten, EK 6,19/3,39/2,39€) — eBay zeigte seit "Preise neu berechnen" "EUR 20,95-EUR 20,95" für alle drei statt drei unterschiedlicher Preise. Root Cause: `computeVariantPriceRows()` berechnete pro Variante bereits korrekt, aber geschrieben wurde nur `safeUniformVariantPrice()` (Maximum) auf JEDE SKU. Neue Funktion `updateEbayVariantPricesIndividually()` (price-monitor.ts) matcht jede Zeile per slugify(attrs) gegen die echten eBay-SKUs und schreibt jeder ihren eigenen Preis; Fallback auf den Einheitswert nur noch pro einzelner nicht zuordenbarer SKU. `recalculate-apply` nutzt die neue Funktion im isVariant-Zweig; `recalculate-preview`-note korrigiert. Committeter Test (`price-monitor.test.ts`) bestätigt 3 unterschiedliche correctSellPrice-Werte. Typecheck + `bun test` (7/7) grün. **Wartet auf:** manuelle Merge-Freigabe (PR als Draft angelegt). **Nach Merge (separater Schritt):** "Preise neu berechnen" muss für alle live gelisteten Varianten-Produkte einmal erneut ausgeführt werden — Fix repariert nichts rückwirkend. Verwandtes, separat angefragtes PR 2/2 (analoger Fix in `/products/check-all-prices`, das aktuell GAR KEINE Varianten-Erkennung hat) noch nicht begonnen.
