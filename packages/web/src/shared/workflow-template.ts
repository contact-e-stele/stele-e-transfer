...
4. **Bei eBay-bestätigter Zustellung ohne vorhandenen Bewertungsbitte-Entwurf**: Entwurf ansehen/erzeugen und mit mir durchgehen. Vor jeder Bewertungsbitte prüfen, ob der Käufer bereits bewertet hat (App prüft das jetzt automatisch über eBay Trading-API) — falls ja, keine weitere Anfrage stellen. Sobald für eine Bestellung eine eBay-Bewertung vom Käufer vorliegt (unabhängig von der Sternezahl — nur Auslöser, kein Sentiment-Abgleich), UND noch keine AliExpress-Bewertung für diese Bestellung abgegeben wurde: bei AliExpress eine kurze, positive Standard-Bewertung hinterlassen ("Gute Qualität, schnelle Lieferung") — schaltet oft Rabatt-Coupons frei. Manueller Browser-Schritt, kein App-Automatismus.
